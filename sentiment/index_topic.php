...
<table width="95%" border="0">
  <tr>
    <td><?php if ($pageNum_topicrs > 0) { // Show if not first page ?>
      <div align="center"> <a href="<?php printf("%s?pageNum_topicrs=%d%s", $currentPage, 0, $queryString_topicrs); ?>">第一頁</a> 
      </div>
      <?php } // Show if not first page ?></td>
    <td> <?php if ($pageNum_topicrs > 0) { // Show if not first page ?>
      <div align="center"> <a href="<?php printf("%s?pageNum_topicrs=%d%s", $currentPage, max(0, $pageNum_topicrs - 1), $queryString_topicrs); ?>">上一頁</a></div>
      <?php } // Show if not first page ?></td>
    <td> <?php if ($pageNum_topicrs < $totalPages_topicrs) { // Show if not last page ?>
      <div align="center"> <a href="<?php printf("%s?pageNum_topicrs=%d%s", $currentPage, min($totalPages_topicrs, $pageNum_topicrs + 1), $queryString_topicrs); ?>">下一頁</a></div>
      <?php } // Show if not last page ?></td>
    <td> <?php if ($pageNum_topicrs < $totalPages_topicrs) { // Show if not last page ?>
      <div align="center"> <a href="<?php printf("%s?pageNum_topicrs=%d%s", $currentPage, $totalPages_topicrs, $queryString_topicrs); ?>">最後一頁</a></div>
      <?php } // Show if not last page ?></td>
  </tr>
</table>
<p>&nbsp;</p>
